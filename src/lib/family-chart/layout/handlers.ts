...
export function sortChildrenWithSpouses(
	children: Datum[],
	datum: Datum,
	data: Data
) {
...
export function setupSiblings({
	...
	function addSiblingsToTree(main: TreeDatum) {
		...
		for (let i = 0; i < siblings.length; i++) {
			...
			const p1 = main.parents!.find(
				(d) => d.data.id === sib.data.rels.parents[0]
			);
			const p2 = main.parents!.find(
				(d) => d.data.id === sib.data.rels.parents[1]
			);
	...
	function positionSiblings(main: TreeDatum) {
		...
		sorted_siblings.sort((a, b) => {
			const a_p1 = main.parents!.find(
				(d) => d.data.id === a.data.rels.parents[0]
			);
			const a_p2 = main.parents!.find(
				(d) => d.data.id === a.data.rels.parents[1]
			);
			const b_p1 = main.parents!.find(
				(d) => d.data.id === b.data.rels.parents[0]
			);
			const b_p2 = main.parents!.find(
				(d) => d.data.id === b.data.rels.parents[1]
			);
		...
		const main_sorted_index = sorted_siblings.findIndex(
			(d) => d.data.id === main.data.id
		);
...
export function getMaxDepth(d_id: Datum['id'], data_stash: Data) {
	...
	const root_ancestry = d3.hierarchy(
		datum,
		(d) => hierarchyGetterParents(d) as Iterable<Datum>
	);
	const root_progeny = d3.hierarchy(
		datum,
		(d) => hierarchyGetterChildren(d) as Iterable<Datum>
	);
