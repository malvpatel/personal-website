...
import {
	handleDuplicateSpouseToggle,
	handleDuplicateHierarchyProgeny
} from '../features/duplicates-toggle/duplicates-progeny';
...
export default function calculateTree(
	...
	const data_stash: Data = single_parent_empty_card
	...
	const tree_children = calculateTreePositions(
		main,
		'children',
		false
	) as TreeDatum[];
	const tree_parents = calculateTreePositions(
		main,
		'parents',
		true
	) as TreeDatum[];
	...
	function calculateTreePositions(
		...
		function sameBothParents(a: HN, b: HN) {
			...
			return (
				parentsA.length === parentsB.length &&
				parentsA.every((p, i) => p === parentsB[i])
			);
		}
		function hasSpouses(d: HN) {
			return d.data.rels.spouses && d.data.rels.spouses.length > 0;
		}
		function someSpouses(a: HN, b: HN) {
	...
	function setupSpouses(tree: TreeDatum[], node_separation: number) {
		for (let i = tree.length; i--; ) {
			...
			if (!d.is_ancestry) {
				...
				if (d._ignore_spouses)
					spouses = spouses.filter(
						(sp_id) => !d._ignore_spouses!.includes(sp_id)
					);
				if (spouses.length > 0) {
					...
					spouses.forEach((sp_id, i) => {
						...
						spouse.sy =
							i > 0 ? spouse.y : spouse.y + (node_separation / 2) * side;
			...
			if (d.parents && d.parents.length === 2) {
				...
				const x = (d: TreeDatum, sp: TreeDatum) =>
	...
	function setupProgenyParentsPos(tree: TreeDatum[]) {
		tree.forEach((d) => {
			...
			if (d.sibling) return;
			const p1 = d.parent;
			const p2 = (p1?.spouses || []).find((d0: TreeDatum) =>
				d.data.rels.parents.includes(d0.data.id)
			);
			if (p1 && p2) {
	...
	function calculateTreeDim(
		...
		if (
			w_extent[0] === undefined ||
			w_extent[1] === undefined ||
			h_extent[0] === undefined ||
			h_extent[1] === undefined
		)
	...
	function createRelsToAdd(data: Data) {
		...
		function findOrCreateToAddSpouse(d: Datum) {
			const spouses = (d.rels.spouses || [])
				.map((sp_id) => data.find((d0) => d0.id === sp_id))
				.filter((d) => d !== undefined);
	...
	function handleDuplicateHierarchy(
		root: HN,
		data_stash: Data,
		is_ancestry: boolean
	) {
...
export function calculateTreeWithV1Data(
	data: LegacyDatum[],
	options: CalculateTreeOptions
) {
