...
export default function pathToMain(
	cards: CardHtmlSelection,
	links: LinkSelection,
	datum: TreeDatum,
	main_datum: TreeDatum
) {
	...
	if (is_ancestry) {
		...
		while (parent !== main_datum && itteration1 < 100) {
			...
			const spouse_link = links_data.find(
				(d) => d.spouse === true && (d.source === parent || d.target === parent)
			);
			if (spouse_link) {
				const child_links = links_data.filter(
					(d) =>
						Array.isArray(d.target) &&
						d.target.includes(spouse_link.source as TreeDatum) &&
						d.target.includes(spouse_link.target as TreeDatum)
				);
				const child_link = getChildLinkFromAncestrySide(
					child_links,
					main_datum
				);
				...
				// single parent
				const child_links = links_data.filter(
					(d) => Array.isArray(d.target) && d.target.includes(parent)
				);
				const child_link = getChildLinkFromAncestrySide(
					child_links,
					main_datum
		...
		links.each(function (d) {
			...
			if (
				...
				links_node_to_main.push({ link: d, node: this });
			if (
				datum.parents.includes(d.source as TreeDatum) &&
				!Array.isArray(d.target) &&
				datum.parents.includes(d.target as TreeDatum)
			)
				links_node_to_main.push({ link: d, node: this });
		});
		...
		while (child !== main_datum && itteration1 < 100) {
			...
			if (child_link) {
				const spouse_link = links_data.find(
					(d) =>
						d.spouse === true &&
						sameArray([d.source, d.target], child_link.source as TreeDatum[])
				)!;
				links_to_main.push(child_link);
				links_to_main.push(spouse_link);
	...
	function getChildLinkFromAncestrySide(
		child_links: Link[],
		main_datum: TreeDatum
	) {
