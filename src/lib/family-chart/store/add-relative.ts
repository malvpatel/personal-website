...
export function addDatumRelsPlaceholders(
	...
	function addParents() {
		...
		const father = parents.find(
			(d_id) => store_data.find((d) => d.id === d_id)?.data.gender === 'M'
		);
		const mother = parents.find(
			(d_id) => store_data.find((d) => d.id === d_id)?.data.gender === 'F'
		);
		if (parents.length < 2 && !father) {
			const father: NewDatum = createNewPerson({
				data: { gender: 'M' },
		...
		if (parents.length < 2 && !mother) {
			const mother: NewDatum = createNewPerson({
				data: { gender: 'F' },
				rels: { children: [datum.id] }
			});
	...
	function addChildren() {
		...
		datum.rels.spouses.forEach((spouse_id) => {
			...
			const new_son: NewDatum = createNewPerson({
				data: { gender: 'M' },
			...
			const new_daughter: NewDatum = createNewPerson({
				data: { gender: 'F' },
				rels: { parents: [datum.id, spouse.id] }
			});
