...
export function handleLinkRel(
	updated_datum: Datum,
	link_rel_id: Datum['id'],
	store_data: Data
) {
	...
	if (link_rel.rels.parents.length === 0) {
		...
		const link_rel_father = link_rel.rels.parents.find(
			(id) => store_data.find((d) => d.id === id)?.data.gender === 'M'
		);
		const link_rel_mother = link_rel.rels.parents.find(
			(id) => store_data.find((d) => d.id === id)?.data.gender === 'F'
		);
		const new_rel_father = new_rel.rels.parents.find(
			(id) => store_data.find((d) => d.id === id)?.data.gender === 'M'
		);
		const new_rel_mother = new_rel.rels.parents.find(
			(id) => store_data.find((d) => d.id === id)?.data.gender === 'F'
		);
		if (new_rel_father) {
			if (link_rel_father) {
				console.error('link rel already has father');
				link_rel.rels.parents[link_rel.rels.parents.indexOf(link_rel_father)] =
					new_rel_father;
		...
		if (new_rel_mother) {
			if (link_rel_mother) {
				...
				link_rel.rels.parents[link_rel.rels.parents.indexOf(link_rel_mother)] =
					new_rel_mother;
	...
	store_data.splice(
		store_data.findIndex((d) => d.id === new_rel_id),
		1
	);
}

export function getLinkRelOptions(datum: Datum, data: Data) {
	const rel_datum = datum._new_rel_data
		? data.find((d) => d.id === datum._new_rel_data.rel_id)
		: null;
	const ancestry_ids = getAncestry(datum, data);
	const progeny_ids = getProgeny(datum, data);
	if (
		datum._new_rel_data &&
		['son', 'daughter'].includes(datum._new_rel_data.rel_type)
	) {
