...
export default function updateCardsHtml(
	...
	const card: CardHtmlSelection = d3
		.select(div)
		.select('.cards_view')
		.selectAll<HTMLDivElement, TreeDatum>('div.card_cont')
		.data(tree.data, (d) => (d as TreeDatum).tid!);
	...
	card_exit.each((d) =>
		calculateEnterAndExitPositions(d as TreeDatum, false, true)
	);
	card_enter.each((d) =>
		calculateEnterAndExitPositions(d as TreeDatum, true, false)
	);
