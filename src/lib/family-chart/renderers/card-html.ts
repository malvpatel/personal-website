...
export default function CardHtml(props: {
    ...
    `;
		this.querySelector('.card')!.addEventListener('click', (e: Event) =>
		...
		if (props.duplicate_branch_toggle)
			handleCardDuplicateToggle(
				this,
				d,
				props.store.state.is_horizontal,
		...
		if (location.origin.includes('localhost')) {
			...
			if (d.data.to_add) {
				...
				if (spouse)
					d3.select(this)
						.select('.card')
						.attr('data-to-add', spouse.data.data['first name']);
			}
		}
	};
	...
	function cardInnerImageCircleRect(d: TreeDatum) {
		return d.data.data[props.cardImageField]
			? cardInnerImageCircle(d)
			: cardInnerRect(d);
	}
	...
	function handleCardDuplicateHover(node: HTMLElement, d: TreeDatum) {
		d3.select(node).on('mouseenter', (e) => {
			d3.select(node.closest('.cards_view'))
				.selectAll('.card_cont')
				.select('.card')
				.classed(
					'f3-card-duplicate-hover',
					(d0) => (d0 as TreeDatum).data.id === d.data.id
				);
		});
		d3.select(node).on('mouseleave', (e) => {
			d3.select(node.closest('.cards_view'))
				.selectAll('.card_cont')
				.select('.card')
				.classed('f3-card-duplicate-hover', false);
		});
	}
}
