...
export function createFormNew(
	form_creator: NewRelFormCreator,
	closeCallback: () => void
) {
	return createForm(form_creator, closeCallback);
}

export function createFormEdit(
...
function createForm(
	...
	function reload() {
		const formHtml = is_new
			? getHtmlNew(form_creator)
			: getHtmlEdit(form_creator);
	...
	function isNewRelFormCreator(
		form_creator: EditDatumFormCreator | NewRelFormCreator
	): form_creator is NewRelFormCreator {
		return 'new_rel' in form_creator;
	}
}

function setupEventListenersBase(
...
function setupEventListenersEdit(
	formContainer: HTMLElement,
	form_creator: EditDatumFormCreator,
	reload: () => void
) {
