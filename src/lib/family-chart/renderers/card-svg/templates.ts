...
export function CardBodyAddNewRel({
	d,
	card_dim,
	label
}: {
...
export function CardBodyOutline({
	d,
	card_dim,
	is_new
}: {
...
export function LinkBreakIconWrapper({
	d,
	card_dim
}: {
	d: TreeDatum;
	...
	let g = '',
		...
		areChildren = (r: TreeDatum['data']['rels']) =>
...
export function CardImage({
	...
	image: string;
	card_dim: CardDim;
	maleIcon?: ({ card_dim }: { card_dim: CardDim }) => string;
	femaleIcon?: ({ card_dim }: { card_dim: CardDim }) => string;
}) {
	return {
		template: `
