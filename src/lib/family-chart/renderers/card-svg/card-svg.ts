...
export default function CardSvg(props: CardSvgProps) {
	...
	return function (this: HTMLElement, d: TreeDatum) {
		...
		if (d.data._new_rel_data) {
			...
			appendTemplate(
				CardBodyAddNewRel({ d, card_dim, label: d.data._new_rel_data.label })
					.template,
	...
	function setupProps(props: CardSvgProps) {
		...
		for (const k in default_props) {
			if (typeof props[k as keyof CardSvgProps] === 'undefined')
				props[k as keyof CardSvgProps] = default_props[
					k as keyof typeof default_props
				] as any;
		}
		return props;
	}
