...
export class CardHtml {
	...
	constructor(cont: HTMLElement, store: Store) {
		...
		this.store = store;
		this.card_display = [
			(d: Datum) => `${d.data['first name']} ${d.data['last name']}`
		];
	...
	setCardInnerHtmlCreator(
		cardInnerHtmlCreator: CardHtml['cardInnerHtmlCreator']
	) {
	...
	onEnterPathToMain(e: Event, datum: TreeDatum) {
		...
		const cards: CardHtmlSelection = d3
			.select(this.cont)
			.select('div.cards_view')
			.selectAll('.card_cont');
		const links: LinkSelection = d3
			.select(this.cont)
