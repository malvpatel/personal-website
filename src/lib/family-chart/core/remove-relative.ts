	...
	activate(datum: Datum) {
		...
		function onChange(
			...
			function handleParentRemoval() {
				...
				parent.rels.children = parent.rels.children.filter(
					(id) => id !== datum.id
				);
			...
			function handleSpouseRemoval(this: RemoveRelative) {
				...
				function openModal(this: RemoveRelative) {
					const current_gender_class =
						datum.data.gender === 'M'
							? 'f3-male-bg'
							: datum.data.gender === 'F'
								? 'f3-female-bg'
								: null;
					const spouse_gender_class =
						spouse.data.gender === 'M'
							? 'f3-male-bg'
							: spouse.data.gender === 'F'
								? 'f3-female-bg'
								: null;
          ...
          `);
				...
				function remove(to_current: boolean) {
					rel_tree_datum.data.rels.spouses =
						rel_tree_datum.data.rels.spouses!.filter((id) => id !== datum.id);
					rels.spouses = rels.spouses!.filter(
						(id) => id !== rel_tree_datum.data.id
					);
					...
					(rels.children || []).forEach((id) => {
						...
						if (child.rels.parents.includes(other_parent.id))
							child.rels.parents = child.rels.parents.filter(
								(id) => id !== other_parent.id
							);
					});
					if (other_parent.rels.children) {
						other_parent.rels.children = other_parent.rels.children.filter(
							(ch_id) => !(childrens_parent.rels.children || []).includes(ch_id)
						);
					}
					onAccept();
				}
			...
			function handleChildrenRemoval() {
				...
				rels.children = rels.children.filter(
					(id) => id !== rel_tree_datum.data.id
				);
				rel_tree_datum.data.rels.parents =
					rel_tree_datum.data.rels.parents.filter((id) => id !== datum.id);
				onAccept();
			}
