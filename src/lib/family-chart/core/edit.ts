...
export class EditTree {
	...
	open(datum: Datum) {
		...
		function handleRemoveRelative(
			self: EditTree,
			tree_datum: TreeDatum | undefined
		) {
	...
	private setupRemoveRelative() {
		...
		function setClass(cont: HTMLElement, add: boolean) {
			d3.select(cont)
				.select('#f3Canvas')
				.classed('f3-remove-relative-active', add);
		}
	}
	...
	cardEditForm(datum: Datum) {
		...
		const form_cont = is_new_rel
			? (this.createFormNew || createFormNew)(
					form_creator as NewRelFormCreator,
					this.closeForm.bind(this)
				)
			: (this.createFormEdit || createFormEdit)(
					form_creator as EditDatumFormCreator,
					this.closeForm.bind(this)
				);
	...
	setLinkExistingRelConfig(
		link_existing_rel_config: EditTree['link_existing_rel_config']
	) {
