...
export default function linkSpouseText(
	svg: SVGElement,
	tree: Tree,
	props: LinkSpouseTextProps
) {
	...
	tree.data.forEach((d) => {
		if (d.coparent && d.data.data.gender === 'F')
			links_data.push({
				nodes: [d, d.coparent],
				id: `${d.data.id}--${d.coparent.data.id}`
			});
		if (d.spouses)
			d.spouses.forEach((sp) =>
				links_data.push({ nodes: [sp, d], id: `${sp.data.id}--${d.data.id}` })
			);
	});
	...
	function linkUpdate(this: SVGGElement, d: LinkDatum) {
		...
		const delay = props.initial
			? calculateDelay(tree, sp1, props.transition_time!)
			: 0;
		...
		text_g
			.transition('text-op')
			.duration(100)
			.delay(delay + props.transition_time!)
			.style('opacity', 1);
	}
