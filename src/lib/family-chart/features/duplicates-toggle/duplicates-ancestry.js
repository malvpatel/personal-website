export function handleDuplicateHierarchyAncestry(
	root,
	...
	function assignDuplicateValues(all_duplicates) {
		...
		if (on_toggle_one_close_others) {
			if (all_duplicates.every((d) => d._toggle < 0)) {
				const first_duplicate = all_duplicates.sort(
					(a, b) => b._toggle - a._toggle
				)[0];
	...
	function checkIfDuplicate(arr1, arr2) {
		return (
			arr1 !== arr2 &&
			arr1.every((d) => arr2.some((d0) => d.data.id === d0.data.id))
		);
	}
