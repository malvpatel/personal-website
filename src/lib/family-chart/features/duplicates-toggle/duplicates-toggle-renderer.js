...
export function handleCardDuplicateToggle(node, d, is_horizontal, updateTree) {
		...
		.on('click', (e) => {
			...
			if (d.spouse) {
				...
				if (!spouse.data._tgdp_sp[parent_id].hasOwnProperty(d.data.id))
					console.error('no toggle', d, spouse);
	...
	d3.select(card)
		...
		.attr(
			'transform',
			toggle_is_off ? 'translate(10.6, 14.5)' : 'translate(4.1, 14.5)'
		)
	...
	if (toggle_is_off) {
		...
		d3.select(card)
			...
			.attr(
				'style',
				'position: absolute; z-index: -1;top: 0;left: 0;border-radius: 0;'
			)
			.style('width', '66px')
			.style('height', '112px')
