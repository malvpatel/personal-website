...
export function handleDuplicateHierarchyProgeny(
	...
	function loopChildren(d) {
		...
		const p1 = d.data;
		const spouses = (d.data.rels.spouses || []).map((id) =>
		...
		spouses.forEach((p2) => {
			if (
				progeny_duplicates.some((d) =>
					d.some((d) => checkIfDuplicate([p1, p2], [d.p1, d.p2]))
				)
			) {
				return;
			}
	...
	function assignDuplicateValues(all_duplicates) {
		all_duplicates.forEach(({ d, p1, p2 }, i) => {
			...
			let val = 1;
			if (!d.data._tgdp_sp[parent_id].hasOwnProperty(p2.id))
				d.data._tgdp_sp[parent_id][p2.id] = val;
		...
		if (on_toggle_one_close_others) {
			...
			if (all_duplicates.filter((d) => d.val > 0).length > 1) {
				const latest_duplicate = all_duplicates.sort(
					(a, b) => b.val - a.val
				)[0];
			...
			if (d.data._tgdp_sp[parent_id][p2.id] < 0) {
				...
				if (children_by_spouse[p2.id]) {
					d.children = d.children.filter(
						(c) => !children_by_spouse[p2.id].includes(c)
					);
