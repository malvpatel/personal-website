...
export default function (cont: HTMLElement) {
	return new Modal(cont);
}
...
export class Modal {
	...
	activate(
		content: string | HTMLElement,
		{
			...
			onAccept,
			onCancel
		}: { boolean?: boolean; onAccept?: () => void; onCancel?: () => void } = {}
	) {
		this.reset();

		const modal_content_inner = d3
			.select(this.modal_cont)
			.select('.f3-modal-content-inner')
			.node()! as HTMLElement;
