...
export function kinshipInfo(
	...
	const relationships = calculateKinships(
		self_id!,
		data_stash,
		kinship_info_config
	);
  ...
  `;
	const kinship_info_node = d3
		.create('div')
		.html(html)
		.select('div')
		.node() as HTMLElement;
	let popup: InfoPopup | null = null;
	d3.select(kinship_info_node)
		.select('.f3-kinship-info-icon')
		.on('click', (e) => createPopup(e, kinship_info_node));
	...
	function createPopup(e: MouseEvent, cont: HTMLElement) {
		...
		const inner_cont = popup.popup_cont.querySelector(
...
function createSmallTree(
	...
	const small_chart_data = getKinshipsDataStash(
		self_id,
		rel_id,
		data_stash,
		relationships
      ...
      `;
			...
			function getCardClass() {
				if (d.data.kinship === 'self') {
					return (
						'card-kinship-self' + (kinship_label_toggle ? '' : ' f3-real-label')
					);
		...
		function createKinshipLabelToggle() {
			kinship_label_toggle_cont.classed('f3-kinship-labels-toggle', true);
