...
export function getKinshipsDataStash(
	...
	if (
		kinship_data_stash.length > 0 &&
		!same_ancestors.is_ancestor &&
		!same_ancestors.is_half_kin
	)
	...
	function getCleanAncestry(
		d_id: Datum['id'],
		same_ancestor_progeny: Datum['id'][]
	) {
		const ancestry = [d_id];
		loopAdd(d_id);
		return ancestry;
	...
	function addInLawSibling(kinship_data_stash: DatumKinship[]) {
		...
		const spouse_id = getD(rel_id)!.rels.spouses?.find((d_id) =>
			siblings.includes(d_id)
		);
		datum.rels.spouses = [spouse_id!];
