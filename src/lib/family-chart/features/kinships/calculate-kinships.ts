...
export function calculateKinships(
	...
	function loopCheck(
		...
		kinship: string,
		depth: number,
		prev_rel_id: Datum['id'] | undefined = undefined
	) {
		if (!d_id) return;
		...
		if (kinship === 'self') {
			rels.parents.forEach((p_id) =>
				loopCheck(p_id, 'parent', depth - 1, d_id)
			);
			...
			rels.parents.forEach((p_id) =>
				loopCheck(p_id, 'grandparent', depth - 1, d_id)
			);
			...
			(rels.children || []).forEach((id) =>
				loopCheck(id, 'grandchild', depth + 1)
			);
			...
			rels.parents.forEach((p_id) =>
				loopCheck(p_id, 'great-grandparent', depth - 1, d_id)
			);
			(rels.children || []).forEach((id) => {
				if (prev_rel_id && prev_rel_id === id) return;
			...
			(rels.children || []).forEach((id) =>
				loopCheck(id, getGreatKinship(kinship, depth + 1), depth + 1)
			);
		} else if (kinship.includes('great-grandparent')) {
			if (!prev_rel_id) console.error(`${kinship} should have prev_rel_id`);
			rels.parents.forEach((p_id) =>
				loopCheck(p_id, getGreatKinship(kinship, depth - 1), depth - 1, d_id)
			);
			(rels.children || []).forEach((id) => {
				if (prev_rel_id && prev_rel_id === id) return;
			...
			(rels.children || []).forEach((id) =>
				loopCheck(id, 'grandnephew', depth + 1)
			);
			...
			(rels.children || []).forEach((id) =>
				loopCheck(id, '1st Cousin', depth + 1)
			);
		} else if (kinship === 'granduncle') {
			(rels.children || []).forEach((id) =>
				loopCheck(id, '1st Cousin 1x removed', depth + 1)
			);
		} else if (kinship.includes('great-granduncle')) {
			const child_depth = depth + 1;
			const removed_count = Math.abs(child_depth);
			(rels.children || []).forEach((id) =>
				loopCheck(id, `1st Cousin ${removed_count}x removed`, child_depth)
			);
			...
			(rels.children || []).forEach((id) => {
				...
				if (child_depth === 0) {
					...
					loopCheck(
						id,
						`${getOrdinal(cousin_count + 1)} Cousin ${removed_count}x removed`,
						child_depth
					);
				} else if (child_depth > 0) {
					loopCheck(
						id,
						`${getOrdinal(cousin_count)} Cousin ${removed_count}x removed`,
						child_depth
					);
				}
			});
		...
		Object.keys(kinships).forEach((d_id) => {
			...
			if (!same_ancestors)
				return console.error(
					`${data_stash.find((d) => d.id === d_id)!.data} not found in main_ancestry`
				);
	...
	function setupInLawKinships(kinships: Kinships, data_stash: Data) {
		Object.keys(kinships).forEach((d_id) => {
			...
			if (kinship === 'spouse') {
				...
				datum.rels.parents.forEach((p_id) =>
					(getD(p_id)!.rels.children || []).forEach((d_id) =>
						siblings.push(d_id)
					)
				);
				siblings.forEach((sibling_id) => {
					if (!kinships[sibling_id]) kinships[sibling_id] = 'sibling-in-law';
				}); // gender label is added in setupKinshipsGender
			}
			...
			if (kinship === 'child') {
				(datum.rels.spouses || []).forEach((spouse_id) => {
					if (!kinships[spouse_id]) kinships[spouse_id] = 'child-in-law';
				}); // gender label is added in setupKinshipsGender
			}

			if (kinship === 'uncle') {
				(datum.rels.spouses || []).forEach((spouse_id) => {
					if (!kinships[spouse_id]) kinships[spouse_id] = 'uncle-in-law';
				}); // gender label is added in setupKinshipsGender
			}

			if (kinship.includes('Cousin')) {
				(datum.rels.spouses || []).forEach((spouse_id) => {
					if (!kinships[spouse_id]) kinships[spouse_id] = `${kinship} in-law`;
				}); // gender label is added in setupKinshipsGender
			}
		});
	}
	...
	function setupKinshipsGender(kinships: Kinships) {
		Object.keys(kinships).forEach((d_id) => {
			...
			if (kinship.includes('parent')) {
				...
				const rel_type =
					gender === 'M' ? 'uncle' : gender === 'F' ? 'aunt' : rel_type_general;
...
export function findSameAncestor(
	main_id: Datum['id'],
	rel_id: Datum['id'],
	data_stash: Data
) {
	...
	function loopCheck(rel_id: Datum['id']) {
		...
		const found_parent = main_ancestry.find(
			(p) =>
				(p[0] && parents[0] && p[0] === parents[0]) ||
				(p[1] && parents[1] && p[1] === parents[1])
		);
		if (found_parent) {
	...
	function checkIfHalfKin(
		ancestors1: AncestryTuple,
		ancestors2: AncestryTuple
	) {
		return (
			ancestors1.some((p, i) => p !== ancestors2[i]) ||
			ancestors2.some((p, i) => p !== ancestors1[i])
		);
	}
}
